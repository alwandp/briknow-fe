            ...
            <nav class="prev-navbar navbar navbar-expand-lg main-navbar">
                <ul class="navbar-nav navbar-right">
                    <li class="w-100 dropdown d-flex justify-content-end">
                        @if($role == 0)
                          @if($data->status == 'draft' || $data->status == null)
                            <a class="btn btn-light box-shadow-none btn-sm mx-1 text-decoration-none" href="{{route('kontribusiimpl.edit',$data->slug)}}"><i class="fas fa-pencil-alt mr-1"></i>Edit</a>
                            ...
                            <button class="btn btn-success box-shadow-none btn-sm mx-1 text-decoration-none" onclick="appr('review', {{$data->id}})"><i class="fas fa-paper-plane mr-1"></i> Send</button>
                          ...
                          @else
                              ...
                              <div class="dropdown-menu dropdown-menu-right px-2 table-responsive" aria-labelledby="information-dropdown">
                                ...
                                <div class="d-flex justify-content-between">
                                    <div class="f-60 text-secondary">
                                      @if($data->status == 'checked' || $data->status == 'approve' || $data->status == 'publish')
                                        {{\Carbon\carbon::create($data->approve_at)->format('d F Y, H.i') ?? '-'}}
                                      @else
                                        {{"-"}}
                                      @endif
                                    </div>
                                ...
                                <div class="d-flex justify-content-between">
                                    <div class="f-60 text-secondary">
                                      @if($data->status == 'approve' || $data->status == 'publish')
                                        {{\Carbon\carbon::create($data->approve_at)->format('d F Y, H.i') ?? '-'}}
                                      @else
                                        {{"-"}}
                                      @endif
                        ...
                        @elseif($role == 3)
                            @if($data->status == "publish")
                              <a class="btn btn-light box-shadow-none btn-sm mx-1 text-decoration-none" href="{{route('kontribusiimpl.edit',$data->slug)}}"><i class="fas fa-pencil-alt mr-1"></i>Edit</a>
                            @elseif ($data->status != "publish")
                              <button class="btn btn-success box-shadow-none btn-sm mx-1 text-decoration-none" onclick="appr('publish', {{$data->id}})">Approve & Publish</button>
                              <a class="btn btn-light box-shadow-none btn-sm mx-1 text-decoration-none" href="{{route('kontribusiimpl.edit',$data->slug)}}"><i class="fas fa-pencil-alt mr-1"></i>Edit</a>
                            @endif
                        @elseif ($role == 1)
                          <button class="btn btn-success box-shadow-none btn-sm mx-1 text-decoration-none" onclick="appr('checked', {{$data->id}})">Approve</button>
                          <button class="btn btn-danger box-shadow-none btn-sm mx-1 text-decoration-none" onclick="reject({{$data->id}})">Reject</button>
                        @elseif ($role == 2)
                          <button class="btn btn-success box-shadow-none btn-sm mx-1 text-decoration-none" onclick="appr('approve', {{$data->id}})">Approve</button>
                          <button class="btn btn-danger box-shadow-none btn-sm mx-1 text-decoration-none" onclick="reject({{$data->id}})">Reject</button>
                        @else
                        ...
                        <div class="col-md-4 col-sm-6" id="prev_const">
                            ...
                            @if ($project->consultant)
                                ...
                                @forelse ($consultants as $consultant)
                                    ...
                                    @endif
                                @empty
                                    <div><span>Internal</span></div>
                                @endforelse
                            @else
                                <div class="konsultan"><span>Internal</span></div>
                            @endif
                        </div>
                        <div class="col-md-5 col-sm-6 justify-content-end">
                            <span class="d-block font-weight-bold">Project Manager</span>
                            <span id="prev_pm"><span>{{!empty($data->project_managers->nama)?$data->project_managers->nama:"(Tidak ada data)"}}</span></span>
                            <small class="d-block fs-12">
                                <i class="far fa-envelope"></i>
                                <a href="{{!empty($data->project_managers->email)?"mailto:".$data->project_managers->email:"#"}}" id="prev_emailpm">{{!empty($data->project->project_managers->email)?\Str::limit($data->project_managers->email,20,'..'):"(Tidak ada data)"}}</a>
                            </small>
                        </div>
                    </div>
                </div>
            ...
            <div class="row" id="desc-preview">
                ...
                @if($data->desc_sosialisasi)
                        <div class="col-md-12 d-block w-100 mb-4 mt-2">
                            ...
                            <div class="metodologi-isi wrap" id="prev_deskripsi">{!! $data->desc_sosialisasi !!}</div>
                        </div>
                        <div class="col-md-12 d-block w-100">
                            <h6>Attachment</h6>
                        </div>

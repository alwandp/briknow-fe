...
function changeDirec() {
  let valDirec = $("#direktorat-lesson-init").val();
  if (valDirec !== "init") {
...
tahapParam = localStorage.getItem('fil_les') || '';
if (tahapParam !== "all") {
  if (tahapParam === "Plan") {
...
function getData(page, tahap, direktorat, divisi, search = "") {
  ...
  if (lastpath === "mylesson") {
    url = `${getCookie('url_be')}api/mylessonlearned?page=${page}&tahap=${tahap}&direktorat=${direktorat}&divisi=${divisi}&search=${search}`;
  } else if (lastpath === "lessonlearned") {
    url = `${getCookie('url_be')}api/mylessonlearned/all?page=${page}&tahap=${tahap}&direktorat=${direktorat}&divisi=${divisi}&search=${search}`;
  }
  console.log(url);
      ...
      if (data.data.data.total !== 0) {
        ...
        for (let i = 0; i < data.data.data.total; i++) {
          if (data.data.data[i].lesson_learned.length !== 0 || data.data.data[i].consultant.length !== 0) {
            $("#container-lesson").append(`
              <div class="card card-lesson-collapse w-100 d-flex mb-4">
                ...
                <div class="row">
                  ...
                  <div class="col-1">
                    <button class="btn btn-outline-secondary fas fa-caret-down" data-toggle="collapse" 
                      ...
                      aria-controls="${data.data.data[i].id}">
                    </button>
                  </div>
                </div>

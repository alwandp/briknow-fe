...
function getDataComsupDiv(){
    ...
    clearLayout()
    const urlArticle = `${getCookie('url_be')}api/get/communicationinitiative/publish/article?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    const urlLogo = `${getCookie('url_be')}api/get/communicationinitiative/publish/logo?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    const urlInfo = `${getCookie('url_be')}api/get/communicationinitiative/publish/infographics?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    const urlTrans = `${getCookie('url_be')}api/get/communicationinitiative/publish/transformation?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    const urlPodcast = `${getCookie('url_be')}api/get/communicationinitiative/publish/podcast?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    const urlVideo = `${getCookie('url_be')}api/get/communicationinitiative/publish/video?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    const urlIg = `${getCookie('url_be')}api/get/communicationinitiative/publish/instagram?page=1&year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}`
    $.ajax({
        url: urlArticle,
        type: "get",
...
function getDataImpl(){
    ...
    clearLayout()
    const urlPilot = `${getCookie('url_be')}api/get/implementation/all/publish/piloting?year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}&page=1`
    const urlRoll = `${getCookie('url_be')}api/get/implementation/all/publish/roll-out?year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}&page=1`
    const urlSosial = `${getCookie('url_be')}api/get/implementation/all/publish/sosialisasi?year=&month=&divisi=${lastpath}&sort=&search=${keywordParam}&page=1`
    $.ajax({
        url: urlPilot,
        type: "get",
        beforeSend: function(xhr){
		...
		$("#row-empty").append(`
		    <div class="p-2 w-100 pt-5 text-center">
			<img src="${uri}/assets/img/forum_kosong_1.png" style="width: 25%; height: fit-conteent">
		    	<h5 class="font-weight-bold mt-5 mb-1">Oops.. Content tidak ditemukan</h5>
			<p class="w-100 text-center font-weight-bold">Coba cari content lain</p>
		    </div>
		`)
                    ...
                    $("#row-video-div").append(`
                         <div class="container-fluid">
                            <div class="card d-flex w-100 p-2" style="border-radius: 16px; width: 30rem">
                                ...
                                <div class="d-flex p-2 justify-content-end">
                                    <div class="row">
                                        <p class="pr-2 fas fa-eye mt-3 grey" style="font-size: 16px; margin-bottom: 0px; margin-top: 0px">
                                            <span>${data.data.data[index].views}</span>
                                        </p>
                                        <button class="btn p-2 grey" style="font-size: 20px" onclick="download(${data.data.data[index].id})">
                                            <img src="${uri+'/assets/img/logo/download_ic.png'}"/>
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            if (data.data.data !== undefined && data.data.data.length !== 0){
		$("#row-empty").remove();
                $("#title-logo-dir").append(`<h6>Sosialisasi</h6>`)
                for (let index=0; index < data.data.data.length; index++) {
                    $("#row-logo-div").append(`
                         <div class="container-fluid">
                            <div class="card d-flex w-100 p-2" style="border-radius: 16px; width: 30rem">
